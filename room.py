...
class RoomBooking:
    def __init__(self, root):
        ...
        rows1 = m_cursor.fetchall()

        # ('Single', 'Double', 'Air-Conditioned Single', 'Air-Conditioned Double', 'Luxury')

        comboroom = ttk.Combobox(labelframeleft, textvariable=self.roomtype, font=('arial', 13, 'bold'), width=25, state='readonly')
        comboroom['values'] = [row[0] for row in rows1]  
        comboroom.current(0)
        ...
        #txtavailroom = ttk.Entry(labelframeleft,textvariable=self.roomavailable, width=27, font=('arial', 13, 'bold'))
        #txtavailroom.grid(row= 4, column= 1)

        conn= mysql.connector.connect(host='localhost', username='root', password='1234', database='Mydata')
        m_cursor= conn.cursor()
        m_cursor.execute('select roomno from roomdetails')
        rows = m_cursor.fetchall()


        comboroomno = ttk.Combobox(labelframeleft, textvariable=self.roomavailable, font=('arial', 13, 'bold'), width=25, state='readonly')
        comboroomno['values'] = [row[0] for row in rows]  
        comboroomno.current(0)
        comboroomno.grid(row=4, column=1)
    ...
    def addData(self):
        if self.varcontact.get()=='' or self.checkin.get()== '':
            ...
            try:
                conn= mysql.connector.connect(host='localhost', username='root', password='1234', database='Mydata')
                m_cursor= conn.cursor()
                m_cursor.execute('insert into room values(%s,%s,%s,%s,%s,%s,%s)', (
                                                                                            self.varcontact.get(),
                                                                                            self.checkin.get(),
                                                                                            self.checkout.get(),
                                                                                            self.roomtype.get(),
    ...
    def search(self):
        ...
        query = f"SELECT * FROM room WHERE {searchcolumn} LIKE %s"
        m_cursor.execute(query, (f'%{search_value}%',))
        rows = m_cursor.fetchall()
